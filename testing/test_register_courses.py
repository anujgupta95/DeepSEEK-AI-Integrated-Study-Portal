...
def test_2_list_registered_courses(student_mail,
    ...
    required_keys = ["registeredCourses"]
    assert set(required_keys) == set(data.keys()), f"Expected response to have following keys: {required_keys}, but found the following keys: {list(data.keys())}"
...
def test_6_register_two_courses(student2_mail,
    ...
    print("Logged in user is ", user_id)
...
def test_1_register_invalid_course(student_mail, 
                          student_id,
                          invalid_course_id,
                          invalid_course_msg):
    input_data = {
